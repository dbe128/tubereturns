...
@Component({
  ...
  template: `
    @if (loading()) {
      ...
      <div class="max-w-screen-2xl mx-auto px-4 md:px-6 py-10">
        ...
        <div class="bg-gray-900 rounded-xl shadow-sm border border-gray-700 p-6 mb-4">
          <div class="flex flex-wrap items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <span class="text-gray-400 text-xs mt-0.5 inline-block">
                youtube.com/@{{ channel()!.handle }}
                @if (lastAnalyzedDate()) {
                  <span class="mx-1.5 text-gray-600">·</span>Last analyzed: {{ lastAnalyzedDate() }}
                }
              </span>
            </div>
        ...
        <div class="bg-gray-900 rounded-xl shadow-sm border border-gray-700 overflow-hidden">
          @if (picksLoading()) {
            ...
            @if (picks().some(p => p.approximatedPrices)) {
              <div class="px-4 py-2 text-xs text-blue-400 border-t border-gray-700">~ Return estimated using AI-approximated price data</div>
            }
      ...
      <a href="https://lightyear.com/profile/bal%C3%A0zs" target="_blank" rel="noopener sponsored"
         class="flex md:hidden flex-col gap-3 mt-4 bg-gray-900 border border-gray-700 hover:border-green-800 rounded-xl px-5 py-4 transition-colors group">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="flex-shrink-0 bg-black rounded-lg p-1.5">
              <img src="https://www.google.com/s2/favicons?sz=64&domain=lightyear.com" alt="Lightyear" class="w-8 h-8">
        ...
        <div class="text-sm text-gray-400">Start investing commission-free — <span class="text-green-400">up to $100 bonus</span> when you sign up with my link.</div>
      </a>
      }
      </div>
      </div>
...
export class ChannelDetailComponent implements OnInit, OnDestroy {
  ...
  readonly lastAnalyzedDate = computed(() => {
    const p = this.picks();
    if (!p.length) { return null; }
    const latest = p.reduce((max, pick) => pick.videoPublishedAt > max ? pick.videoPublishedAt : max, p[0].videoPublishedAt);
