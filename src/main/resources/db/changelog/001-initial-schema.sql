...
--comment: Create initial schema — channels, videos, picks, pick_prices
...
CREATE TABLE pick_prices (
    id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    pick_id     BIGINT NOT NULL UNIQUE,
    price_date  DATE NOT NULL,
    close_price DOUBLE NOT NULL,
    created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_pick_prices_pick FOREIGN KEY (pick_id) REFERENCES picks(id) ON DELETE CASCADE
);

CREATE INDEX idx_pick_prices_pick_id ON pick_prices(pick_id);
CREATE INDEX idx_pick_prices_price_date ON pick_prices(price_date);
